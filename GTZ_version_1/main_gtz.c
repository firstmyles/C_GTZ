...
void clk_SWI_GTZ_All_Freq(UArg arg0) {
	...
	static int Q1[8] = {0};
	static int Q2[8] = {0};
	static int Q1_square;
	static int Q2_square;
	static int Q1Q2_coef;
	static short input;
   	//Record start time
	...
	if(N == 206){
		...
		for(j=0; j<8; j++){
		     Q1_square = (Q1[j]*Q1[j]);
		     Q2_square = (Q2[j]*Q2[j]);
		     Q1Q2_coef = (Q1[j]*Q2[j]*(coef[j] >> 14));
		     Goertzel_Value = Q1_square + Q2_square - Q1Q2_coef;
		     Goertzel_Value = Goertzel_Value >> 8;

		     gtz_out[j] = sqrt(Goertzel_Value);
		}
